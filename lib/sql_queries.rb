...
"select p.category, sum(pl.amount) as amount from projects p left outer join pledges pl on p.id = pl.project_id and p.category = 'music' group by p.category order by amount desc, p.category"
end
